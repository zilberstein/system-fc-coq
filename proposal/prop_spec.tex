...
\end{tikzpicture}
\begin{center}
\it Figure 2: Block diagram depicting the anticipated approach. The arrows on the left side represent work required to complete each given step.
\end{center}

The first feature that will be added to the formalization of System F is type coercions without datatypes.  Coercions are responsible for most of the power of System FC over System F. They allow a conversion from one type to another. A basic example of the usefulness of coercions is provided in Figure 3.
\begin{verbatim}
data G a where
  G1 :: G Int
  G2 :: G Bool
...
\begin{lstlisting}
G :: * $\rightarrow$ *
G1 :: $\forall$ (a :: *). a ~ Int $\rightarrow$ G a
G2 :: $\forall$ (a :: *). a ~ Bool $\rightarrow$ G a
...
\end{lstlisting}
\begin{center}
\it Figure 3: Coercion example. This example shows how coercions are needed to translate Haskell code (top) to System FC (bottom)
\end{center}
...
One specific challenge may arise regarding the proofs of progress, preservation, and soundness in some instances after coercions, datatypes, and type families have been. Recall Figure 3. Here, a coercion could be added in the scrutinee of the case statement of \texttt{f}, where the scrutinee of a case statement represents that which is being broken down into cases. With this addition, progress, preservation, and soundness may be very difficult to prove.
